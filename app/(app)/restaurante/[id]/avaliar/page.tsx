...
const safetyOptions = [
  { value: "muito_seguro" as SafetyChoice, dot: "#43A047", color: "#2E7D32", bg: "#E8F5E9", border: "#43A047" },
  { value: "seguro"       as SafetyChoice, dot: "#F59E0B", color: "#D97706", bg: "#FFFBEB", border: "#F59E0B" },
  { value: "moderado"     as SafetyChoice, dot: "#FB8C00", color: "#E65100", bg: "#FFF3E0", border: "#FB8C00" },
  { value: "nao_seguro"   as SafetyChoice, dot: "#EF5350", color: "#C62828", bg: "#FFEBEE", border: "#EF5350" },
];
const contamOptions = [
  { value: "nenhum"  as ContamChoice, dot: "#43A047", color: "#2E7D32", bg: "#E8F5E9", border: "#43A047" },
  { value: "pequeno" as ContamChoice, dot: "#F59E0B", color: "#D97706", bg: "#FFFBEB", border: "#F59E0B" },
  { value: "alto"    as ContamChoice, dot: "#EF5350", color: "#C62828", bg: "#FFEBEE", border: "#EF5350" },
  { value: "nao_sei" as ContamChoice, dot: "#9E9E9E", color: "#616161", bg: "#F5F5F5", border: "#9E9E9E" },
];
const teamOptions = [
  { value: "total"         as TeamChoice, dot: "#43A047", color: "#2E7D32", bg: "#E8F5E9", border: "#43A047" },
  { value: "parcial"       as TeamChoice, dot: "#F59E0B", color: "#D97706", bg: "#FFFBEB", border: "#F59E0B" },
  { value: "nao_sabiam"    as TeamChoice, dot: "#EF5350", color: "#C62828", bg: "#FFEBEE", border: "#EF5350" },
  { value: "nao_perguntei" as TeamChoice, dot: "#9E9E9E", color: "#616161", bg: "#F5F5F5", border: "#9E9E9E" },
];
...
function AccordionItem({
  icon: Icon, iconColor = "#FC6904", title, subtitle, summary, open, onToggle, required, done, respondLabel = "Responder", children,
}: {
  icon: React.ElementType; iconColor?: string; title: string; subtitle: string;
  summary?: string; open: boolean; onToggle: () => void;
  required?: boolean; done?: boolean; respondLabel?: string; children: React.ReactNode;
}) {
  return (
  ...
  if (submitted) {
    return (
      ...
      >
        ...
        <h1 className="text-[24px] font-extrabold text-text-primary mb-2">{t.avaliar.successTitle}</h1>
        <p className="text-text-secondary text-sm leading-relaxed mb-2">
          {t.avaliar.successMessageLine1}<br />{t.avaliar.successMessageLine2}
        </p>
        <p className="text-text-disabled text-xs mb-8">
          {t.avaliar.successNote}
        </p>
  ...
  return (
    <div className="bg-background min-h-dvh" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 100px)", fontFamily: "var(--font-nunito), 'Nunito', sans-serif" }}>
      ...
      >
        ...
        <div>
          <h1 className="font-extrabold text-[18px] leading-tight" style={{ color: "#1F3D34" }}>{t.avaliar.headerTitle}</h1>
          <p className="text-[12px] text-text-disabled mt-0.5">{t.avaliar.headerSubtitle}</p>
        </div>
      </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 pb-5">
          ...
          <div className="flex flex-col gap-1 mt-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <p className="text-white/80 text-xs">{t.avaliar.chefLabel.replace("{name}", restaurant.name.split(" ")[0])}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mx-5 mt-4 rounded-2xl px-4 py-3 flex items-center gap-3" style={{ backgroundColor: "#1F3D34" }}>
        ...
        <p className="text-sm font-semibold flex-1" style={{ color: "#C6F59D" }}>{t.avaliar.visitDatePrompt}</p>
      ...
      <div className="px-5 py-4 space-y-3">
        ...
        <AccordionItem icon={Shield} iconColor="#2E7D32" title={t.avaliar.sections.safety.title} subtitle={t.avaliar.sections.safety.subtitle}
          summary={safety ? t.avaliar.safetyLabels[safety] : undefined} respondLabel={t.avaliar.respondButton}
          open={openSection === "safety"} onToggle={() => toggle("safety")} required done={safety !== null}>
          <div className="space-y-2">
            {safetyOptions.map((opt) => (
              <OptionBtn key={String(opt.value)} active={safety === opt.value} {...opt} label={t.avaliar.safetyLabels[opt.value as string]}
        ...
        <AccordionItem icon={AlertTriangle} iconColor="#E65100" title={t.avaliar.sections.contam.title} subtitle={t.avaliar.sections.contam.subtitle}
          summary={contam ? t.avaliar.contamLabels[contam] : undefined} respondLabel={t.avaliar.respondButton}
          open={openSection === "contam"} onToggle={() => toggle("contam")} required done={contam !== null}>
          <div className="space-y-2">
            {contamOptions.map((opt) => (
              <OptionBtn key={String(opt.value)} active={contam === opt.value} {...opt} label={t.avaliar.contamLabels[opt.value as string]}
        ...
        {/* 3. Equipe */}
        <AccordionItem icon={Users} iconColor="#6A1B9A" title={t.avaliar.sections.team.title} subtitle={t.avaliar.sections.team.subtitle}
          summary={team ? t.avaliar.teamLabels[team] : undefined} respondLabel={t.avaliar.respondButton}
          open={openSection === "team"} onToggle={() => toggle("team")} done={team !== null}>
          <div className="space-y-2">
            {teamOptions.map((opt) => (
              <OptionBtn key={String(opt.value)} active={team === opt.value} {...opt} label={t.avaliar.teamLabels[opt.value as string]}
        ...
        {/* 4. Estrutura */}
        <AccordionItem icon={Building2} iconColor="#1565C0" title={t.avaliar.sections.structure.title} subtitle={t.avaliar.sections.structure.subtitle}
          summary={structure.length > 0 ? structure.map(s => t.avaliar.structureOptions[s] ?? s).join(", ") : undefined} respondLabel={t.avaliar.respondButton}
            ...
            {structureOptions.map((opt) => {
              ...
              return (
                <button key={opt} onClick={() => setStructure(p => active ? p.filter(x => x !== opt) : [...p, opt])}
                  ...
                  {t.avaliar.structureOptions[opt] ?? opt}
                </button>
              );
            })}
          </div>
        ...
        {/* 5. Pratos */}
        <AccordionItem icon={UtensilsCrossed} iconColor="#00695C" title={t.avaliar.sections.dishes.title} subtitle={t.avaliar.sections.dishes.subtitle}
          summary={dishes.filter(Boolean).join(", ") || undefined} respondLabel={t.avaliar.respondButton}
          ...
          <div className="space-y-2">
            {dishes.map((dish, i) => (
              <div key={i} className="flex items-center gap-2">
                <input value={dish} onChange={(e) => setDishes(p => p.map((d, idx) => idx === i ? e.target.value : d))}
                  placeholder={t.avaliar.dishPlaceholder.replace("{n}", String(i + 1))}
            ...
            <button onClick={() => setDishes(p => [...p, ""])}
              ...
              <Plus size={15} /> {t.avaliar.addDish}
            </button>
          </div>
        </AccordionItem>

        {/* 6. Nota geral + comentário */}
        <AccordionItem icon={Star} iconColor="#D97706" title={t.avaliar.sections.stars.title} subtitle={t.avaliar.sections.stars.subtitle}
          summary={stars > 0 ? (stars === 1 ? t.avaliar.starsSummary.singular.replace("{count}", "1") : t.avaliar.starsSummary.plural.replace("{count}", String(stars))) : undefined} respondLabel={t.avaliar.respondButton}
          open={openSection === "stars"} onToggle={() => toggle("stars")} done={stars > 0}>
          <div className="flex flex-col items-center gap-2 mb-3">
            <StarRow value={stars} onChange={setStars} />
            <p className="text-text-disabled text-xs">
              {t.avaliar.starLabels[stars]}
            </p>
          </div>
          <textarea value={comment} onChange={(e) => setComment(e.target.value)}
            placeholder={t.avaliar.commentPlaceholder}
            rows={3}
        ...
        {/* 7. Foto */}
        <AccordionItem icon={Camera} iconColor="#AD1457" title={t.avaliar.sections.photo.title} subtitle={t.avaliar.sections.photo.subtitle}
          respondLabel={t.avaliar.respondButton} open={openSection === "photo"} onToggle={() => toggle("photo")}>
          <button className="w-full h-20 rounded-xl border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 active:bg-background transition-colors">
            ...
            <span className="text-text-disabled text-xs">{t.avaliar.photoCta}</span>
          </button>
        </AccordionItem>

        {/* 8. Tags rápidas */}
        <AccordionItem icon={Shield} iconColor="#FC6904" title={t.avaliar.sections.tags.title} subtitle={t.avaliar.sections.tags.subtitle}
          summary={tags.length > 0 ? (tags.length === 1 ? t.avaliar.tagsSelected.singular.replace("{count}", "1") : t.avaliar.tagsSelected.plural.replace("{count}", String(tags.length))) : undefined} respondLabel={t.avaliar.respondButton}
        ...
        <button onClick={handleSubmit} disabled={!canSubmit || loading}
          ...
          {loading ? t.avaliar.submitting : t.avaliar.submit}
        </button>

        {!canSubmit && (
          <p className="text-center text-text-disabled text-xs -mt-2">
            {t.avaliar.submitHint}
          </p>
        )}
      </div>
    </div>
